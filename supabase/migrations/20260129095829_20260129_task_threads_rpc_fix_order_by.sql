/*
  # Fix Task Master RPC Function - Fix ORDER BY

  1. Changes
    - Fix ORDER BY clause to reference fields from the subquery alias
*/
...
BEGIN
  ...
  SELECT json_agg(deal_obj ORDER BY deal_obj.stage, deal_obj.name) INTO result
  FROM (
    SELECT
      o.id,
      o.name,
